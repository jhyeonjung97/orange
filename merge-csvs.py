...
for subdir in subdirs:
    subdir_path = os.path.join(parent_dir, subdir)
    subdir_data = pd.concat([pd.read_csv(os.path.join(subdir_path, f)) for f in os.listdir(subdir_path) if f.endswith('.csv')])
    data.append(subdir_data)
    
# Concatenate all the dataframes in the list and save to a new CSV file
concatenated_data = pd.concat(data)
concatenated_data.to_csv('merged.csv', index=False)
